...
class Project < ActiveRecord::Base
  ...
  def refresh
    ...
    begin
      ...
      current_iteration_number = doc.at('current_iteration_number').innerHTML.strip
      self.name = project_name unless self.name == project_name

      self.iteration_duration_weeks = doc.at('iteration_length').innerHTML
      fetch_current_iteration(iteration_start_day_of_week, current_iteration_number.to_i) unless self.new_record?
...
#    the_date
  ...
  def self.last_read(limit, user)
    projects = Project.joins("LEFT OUTER JOIN user_projects ON user_projects.project_id = projects.id and user_projects.user_id = #{user.id}").where(:tenant_id => user.tenant.id).order('user_projects.read_at DESC').order(:name).limit(limit).sort_by { |p| p.name }
  end
